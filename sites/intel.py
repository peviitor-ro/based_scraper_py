...
from getCounty import remove_diacritics, get_county
...
def get_aditional_city(url):
    ...
    locations = scraper.find(
        'span', {'class': 'job-description__location-pin'}).text.split('|')
...
for job in jobs:
    ...
    if "Multiple Locations" in cities:
        ...
        cities = remove_diacritics(
            locations[0].strip()
        )
        counties = remove_diacritics(
            locations[1].strip()
        )
