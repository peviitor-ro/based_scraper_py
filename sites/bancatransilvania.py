...
for job in response:
    ...
    job_link = 'https://www.ejobs.ro/user/locuri-de-munca/' + \
        job.get('slug') + '/' + str(job.get('id'))
    country = 'Romania'
    ...
    for location in locations:
        try:
            city = translate_city(location.get(
                'address').split(',')[0].strip())
            county = get_county(city)

            if not county:
                city = location.get('address').split(
                    ',')[0].strip().replace(' ', '-')
                county = get_county(city)
...
publish_logo(
    company, 'https://www.bancatransilvania.ro/themes/bancatransilvania/assets/images/logos/bt-cariere.svg')
